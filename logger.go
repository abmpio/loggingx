...
type Config struct {
	...
	// LogRotateDuration is the user specified time to ratate logs
...
func Setup(config Config, out io.Writer) (hclog.InterceptLogger, error) {
	...
	// If out is os.Stdout and app is being run as a Windows Service, writes will
	...
	if config.EnableSyslog {
		...
		delay := 5 * time.Second
		for i := 0; i <= retries; i++ {
			syslog, err := gsyslog.NewLogger(gsyslog.LOG_NOTICE, config.SyslogFacility, "hid")
			if err == nil {
